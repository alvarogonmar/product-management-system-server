...
describe("GET /api/productos/:id", () => {
    it("should return a 404 response for a non-existing product", async () => {
        ...
        expect(response.status).toBe(404); // Expects not found status
...
describe("PATCH /api/productos/:id", () => {
    it("should return a 404 response for a non existent product", async () => { // Test case for a non-existent product
        const productID = 2000 // Define a product ID that does not exist
        const response = await request(server) // Make a PATCH request to the endpoint with the non-existent product ID
            .patch(`/api/productos/${productID}`); // Send the request without a body since PATCH is used to update availability

        expect(response.status).toBe(404); // Expect the response status to be 404 (Not Found)
        expect(response.body.error).toBe("Product not found"); // Expect the response body to contain an error message indicating the product was not found
        expect(response.body).not.toHaveProperty("data"); // Ensure that the response body does not contain a data property since the product does not exist
    });
