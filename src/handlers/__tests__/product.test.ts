...
describe("PATCH /api/productos/:id", () => {
    it("should return a 404 response for a non existent product", async () => { // Missing ID validation test
        const productID = 2000 // Non-existent product ID
        const response = await request(server) // Make the PATCH request
            .patch(`/api/productos/${productID}`); //PATCH endpoint to toggle availability

        expect(response.status).toBe(404); // Expect a 404 Not Found status
        expect(response.body.error).toBe("Product not found"); // Expect the error message

        expect(response.status).not.toBe(200); // Ensure status is not 200
    ...
    it("should update the product availability", async () => { // Test to toggle product availability
        ...
        expect(response.status).not.toBe(404); // Ensure status is not 404
        expect(response.status).not.toBe(400);
