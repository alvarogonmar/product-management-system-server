...
describe("PATCH /api/productos/:id", () => {
    it("should return a 404 response for a non existent product", async () => {  // Test for non-existent product
        ...
        expect(response.body).not.toHaveProperty("data"); // Ensure no data property
    });

    it("should update the product availability", async () => { // Test for updating availability
        const response = await request(server)
            .patch(`/api/productos/1`);

        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty("data"); // Check for data property
        expect(response.body.data.availability).toBe(false); // Check if availability is updated
